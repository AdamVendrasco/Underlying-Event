...
for pdgIds, pts, etas, phis, masses in zip(pdgId_array, pt_array, eta_array, phi_array, mass_array):
    muon_count = 0
    for pdgId, pt, eta in zip(pdgIds, pts, etas):
        if abs(pdgId) == 13 and pt > 2.0 and abs(eta) < 2.5:
            muon_count += 1
    if muon_count == 2:
        total_pz_non_muons = 0
        total_pz_muon = 0
        for pdgId, pt, eta, phi, mass in zip(pdgIds, pts, etas, phis, masses):
            if abs(pdgId) ==13:
                tlv_muon = TLorentzVector()
                tlv_muon.SetPtEtaPhiM(pt, eta, phi, mass)
                total_pz_muon += tlv_muon.Pz()
                combined_muon_pz_array.append(total_pz_muon)
            
            if abs(pdgId) != 13:
                tlv_non_muon = TLorentzVector()
                tlv_non_muon.SetPtEtaPhiM(pt, eta, phi, mass)
                total_pz_non_muons += tlv_non_muon.Pz()
                combined_non_muon_pz_array.append(total_pz_non_muons)


#The combined muon pz are my labels!
combined_muon_pz_array= np.array(combined_muon_pz_array)
combined_non_muon_pz_array=np.array(combined_muon_pz_array)
