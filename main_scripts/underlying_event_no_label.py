    ...
    if num_muons == 2:
        ...
        for pt_val, pdgId, eta_val, phi_val, mass_val in zip(pt, pdgIds, eta, phi, mass):
            ...
            if abs(pdgId) != 13:
                ...
                non_muon_pz_array.append(tlv_non_muon.Pz())
                non_muon_particles = np.append(non_muon_particles, [[pt_val, pdgId, eta_val, phi_val]], axis=0)

print("Total amount of non_muons: ", len(non_muon_pz_array))
print("Total amount of muons: ", len(muon_pz_array))

#########################################
# Start of the Model creation and tranining
#########################################

X_train, X_test = train_test_split(non_muon_pz_array, test_size=0.2, random_state=23)
